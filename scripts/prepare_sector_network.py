...
#         demaind_config = year_config[demand_type]

#         s_util = demaind_config['s_util']
#         s_flex = demaind_config['s_flex']
#         s_inc = demaind_config['s_inc']
#         s_dec = demaind_config['s_dec']
#         delta_t =  demaind_config['delta_t']
        
#         nodes = n.loads[(n.loads['carrier'] == 'electricity') & (n.loads.index.str.startswith('DE'))].index

#         load_p_t = n.loads_t['p_set'][nodes]
...
#         p_max_pu = p_max_t.clip(lower=0).div(lam) 
#         p_min_pu = p_min_t.clip(upper=0).div(lam)

#         name =  " " + demand_type + ' DSM'

#         p_max_pu_df = p_max_pu.add_suffix(name)
#         p_min_pu_df = p_min_pu.add_suffix(name)
#         lam.index = lam.index.map(lambda x: x + name)

       
#         n.add(
#             "StorageUnit",
#             nodes + " " + demand_type + ' DSM',
...
def add_low_voltage_dsm (n, config, planning_horizons, sector_opts):
    ...
    for demand_type in year_config:
        ...
        p_max_pu_df.columns = location_names + f" {demand_type} DSM dispatch"
        ...
        lam.index = location_names + f" {demand_type} DSM store"
        p_max_pu_df.columns = location_names + f" {demand_type} DSM store"
        p_min_pu_df.columns = location_names + f" {demand_type} DSM store"

        n.add(
            "Link",
            location_names + f" {demand_type} DSM store",
